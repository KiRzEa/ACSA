...
def main():
    ...
    no_response_format = set()   # models/providers that reject response_format=json_object are remembered here
    ...
    def call(e):
        ...
        for attempt in range(8):
            try:
                limiter.wait()
                kw = dict(model=a.model, messages=[{"role": "system", "content": system}, {"role": "user", "content": e.text}])
                if send_store["v"]:
    ...
    stats = {"domain": a.domain, "model": a.model, "effort": a.reasoning_effort, "n": len(test), "P": P, "R": R, "F1": F,
             ...
             "avg_reasoning_tokens": sum(r["reasoning_tokens"] for r in rs) / n, "provider": a.provider,
