...
# files are re-packed to /kaggle/working/cage_results_queue_<round>_s<N>.zip after every job. Unzip each zip into ./outputs/ locally.
set -uo pipefail
cd "$(dirname "$0")"
...
bash scripts/pack_results.sh "$(python3 scripts/kaggle_queue.py run --session "$N" --tag)"
