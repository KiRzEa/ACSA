#!/usr/bin/env bash
# Beauty: viT5-large only (both GPUs, model-parallel via --device_map_auto).
# Split into its own session/script -- see run_baselines_beauty_mt5large.sh
# for why these two run separately rather than back-to-back in one session.
set -uo pipefail
cd "$(dirname "$0")/.." || exit 1
...
python3 baselines/t5_seq2seq_baseline.py --train_path "$TRAIN" --dev_path "$DEV" --test_path "$TEST" \
    ...
    --batch_size 8 --eval_batch_size 4 --epochs 10 --gradient_checkpointing --device_map_auto
...
echo "[$(date '+%H:%M:%S')] Beauty viT5-large finished."
