#!/usr/bin/env bash
# Beauty: mT5-large only (both GPUs, model-parallel via --device_map_auto).
...
# See run_baselines_beauty_vit5large.sh for the other one.
set -uo pipefail
cd "$(dirname "$0")/.." || exit 1
...
echo "[$(date '+%H:%M:%S')] Beauty mT5-large finished."
